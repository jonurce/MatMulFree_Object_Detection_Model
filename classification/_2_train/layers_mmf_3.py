...
################ MMF Linear Function ################
class MMFLinearFunctionv6(torch.autograd.Function):
    # Theory: X ∈ R^{M×N}, W ∈ R^{N×K}, b ∈ R^K
    # Code: X [M,N], W [K, N], b [K] 
    # Step 1. Load from HBM: X, W, b (HBM slow)
    @staticmethod
    def forward(ctx, X, W, b, scale_w):
        # Step 2. On chip: Y_tilde, mu, var, r <- rms_norm_fwd(X)
        mu   = X.mean(dim=-1, keepdim=True)             # [M, 1]
        var  = X.var(dim=-1, keepdim=True, unbiased=False) # [M, 1]
        r    = 1.0 / (var + 1e-8).sqrt()                 # [M, 1]
...
class MMFLinearv6(nn.Module):
    ...
    def forward(self, x):
        return MMFLinearFunctionv6.apply(x, self.weight, self.bias, self.scale_w)

################ MMF Conv2d Function ################
class MMFConv2dFunction6(torch.autograd.Function):
    # Theory: X ∈ R^{B×C_in×H_in×W_in}, W ∈ R^{C_out×C_in×kH×kW}, b ∈ R^{C_out}
    # Code: X [B, C_in, H_in, W_in], W [C_out, C_in, kH, kW], b [C_out]
    ### Step 1. Load from HBM: X, W, b (HBM slow)
    @staticmethod
    def forward(ctx, X, W, b, stride, padding, scale_w):
    ...
    def backward(ctx, dO):
        ...
        ### Step 6: Store dX, dW, db to HBM
        # Same args as forward(ctx, X, W, b, stride, padding)
        return dX, dW, db, None, None, dscale_w   # None for stride, padding, scale_w
...
class MMFConv2dv6(nn.Module):
    ...
    def forward(self, x):
        return MMFConv2dFunction6.apply(x, self.weight, self.bias, self.stride, self.padding, self.scale_w)
...
################ MMF Linear Function ################
class MMFLinearFunctionv7(torch.autograd.Function):
    # Theory: X ∈ R^{M×N}, W ∈ R^{N×K}, b ∈ R^K
    # Code: X [M,N], W [K, N], b [K] 
    # Step 1. Load from HBM: X, W, b (HBM slow)
    @staticmethod
    def forward(ctx, X, W, b, half_levels):
        # Step 2. On chip: Y_tilde, mu, var, r <- rms_norm_fwd(X)
        mu   = X.mean(dim=-1, keepdim=True)             # [M, 1]
        var  = X.var(dim=-1, keepdim=True, unbiased=False) # [M, 1]
        r    = 1.0 / (var + 1e-8).sqrt()                 # [M, 1]
    ...
    def backward(ctx, dO):
        ...
        # Step 6: Store dX, dW, db to HBM
        # Same args as forward(ctx, X, W, b)
        return dX, dW, db, None   # None for half_levels

################ MMF Linear Layer ################
class MMFLinearv7(nn.Module):
    def __init__(self, in_features, out_features, weight_init_scale=1, num_levels=3):
        super().__init__()
        
        in_features = int(in_features)
        out_features = int(out_features)

        # num_levels: 3 = ternary, 5 = quinary, 7 = etc.
        self.half_levels = float((num_levels - 1) // 2)  # 2 for quinary, 1 for ternary
    ...
    def forward(self, x):
        return MMFLinearFunctionv7.apply(x, self.weight, self.bias, self.half_levels)

################ MMF Conv2d Function ################
class MMFConv2dFunctionv7(torch.autograd.Function):
    # Theory: X ∈ R^{B×C_in×H_in×W_in}, W ∈ R^{C_out×C_in×kH×kW}, b ∈ R^{C_out}
    # Code: X [B, C_in, H_in, W_in], W [C_out, C_in, kH, kW], b [C_out]
    ### Step 1. Load from HBM: X, W, b (HBM slow)
    @staticmethod
    def forward(ctx, X, W, b, stride, padding, half_levels):
    ...
    def backward(ctx, dO):
        ...
        ### Step 6: Store dX, dW, db to HBM
        # Same args as forward(ctx, X, W, b, stride, padding)
        return dX, dW, db, None, None, None   # None for stride, padding, half_levels

################ MMF Conv2d Layer ################
class MMFConv2dv7(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, weight_init_scale=1, num_levels=3):
        super().__init__()
    ...
    def forward(self, x):
        return MMFConv2dFunctionv7.apply(x, self.weight, self.bias, self.stride, self.padding, self.half_levels)
...
if __name__ == "__main__":
    ...
    lin_mmf = MMFLinearv6(128, 64).cuda()
    out_mmf = lin_mmf(x)
    ...
    conv_mmf = MMFConv2dv6(3, 64, kernel_size=3, stride=1, padding=1).cuda()
    out_mmf = conv_mmf(x)
    ...
    layer = MMFConv2dv6(3, 64, kernel_size=3, padding=1).cuda()
    fc = MMFLinearv6(64, 10).cuda()
