...
__global__ void mmf_linear_kernel(
    ...
    // Shared memory tiles for x and weight
    // Each block loads TILE rows of x and TILE rows of weight at a time
    ...
    for (int t = 0; t < num_tiles; t++) {
        ...
        // Load x tile: x[b_row, feat_idx] where b_row = blockIdx.y * TILE + threadIdx.y
        int b_row = blockIdx.y * TILE + threadIdx.y;
        x_tile[threadIdx.y][threadIdx.x] =
            (b_row < B && feat_idx < in_features)
            ? x[b_row * in_features + feat_idx]
            : 0.0f;

        // Load weight tile: weight[o_row, feat_idx] where o_row = blockIdx.x * TILE + threadIdx.y
        int o_row = blockIdx.x * TILE + threadIdx.y;
        w_tile[threadIdx.y][threadIdx.x] =
            (o_row < out_features && feat_idx < in_features)
            ? weight[o_row * in_features + feat_idx]
            : 0.0f;
        ...
        // Sync: ensure all threads are done computing before next tile load
        __syncthreads();
    }
